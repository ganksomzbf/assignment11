<template>
    ...
    <section class="about">
      ...
      >
        <template #action="{ record }">
          ...
          <a href="javascript:void(0)" @click="showConfirmDelete(record.id)"
            >ลบ</a
          >
        </template>
      </a-table>
    ...
    >
      ...
      <br />
      <a-row>
        <a-col :span="6"> <label>ราคา</label> </a-col>
        <a-col :span="18">
  ...
  export default {
    ...
    methods: {
      ...
      async handleOk() {
        try {
          let res = this.book.id
            ? await axios.put(
                "http://localhost:3000/books/" + this.book.id,
                this.book
              )
            : await axios.post("http://localhost:3000/books", this.book);
          if (res) this.modalVisible = false;
        } catch (error) {
        } finally {
          this.getBooks();
        }
